...
const ChatInterface = ({ embedded = false }) => {
  ...
  const user = location.state?.user || { role: 'parent', name: 'Parent' };
  ...
  const sendMessage = async (text = input) => {
    ...
    try {
      const contextPrefix = `[System: The current authenticated user is ${user.name}. Keep this context in mind if they ask for their child's grades or attendance. Assume their child's name is "John Doe" unless specified.]\n`;
    ...
    <div className={`flex w-full gap-6 text-slate-200 ${embedded ? 'h-full p-0' : 'h-screen p-4 md:p-8 max-w-7xl mx-auto'}`}>
      ...
      >
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <div className="mt-4 px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-full text-amber-400 text-xs font-semibold tracking-wider flex items-center gap-2">
            <User size={14} />
            {user.name} ({user.role})
          </div>
        </div>
        ...
        <div className="mt-auto space-y-4 pt-6 border-t border-white/10">
          ...
          <div>
            ...
            <select 
              ...
              className="w-full bg-black/30 border border-white/10 rounded-xl p-3 outline-none focus:border-amber-500 transition-colors"
            >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 opacity-50">
              ...
              <p>Start a conversation with Aether (Parent Support)</p>
            </div>
          )}
          <AnimatePresence>
            {messages.map((msg, index) => (
              ...
              >
                {msg.sender === 'ai' && (
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-rose-500 flex items-center justify-center flex-shrink-0 text-white font-bold text-xs shadow-lg shadow-rose-500/20">
                    A
                  </div>
                )}
                  ...
                  <div 
                    className={`p-4 ${
                      msg.sender === 'user' 
                        ...
                        : 'glass-card rounded-2xl rounded-tl-sm border-l-2 border-l-amber-400 shadow-lg'
                    }`}
                  >
        ...
        {/* Input Box */}
        <div className="p-6">
          <div className="relative flex items-center bg-black/40 border border-white/10 rounded-2xl p-2 shadow-lg backdrop-blur-md focus-within:border-amber-500/50 transition-colors">
            ...
            <button 
              onClick={() => sendMessage()}
              className="px-6 py-3 bg-gradient-to-r from-amber-500 to-rose-600 hover:from-amber-400 hover:to-rose-500 text-white rounded-xl font-medium transition shadow-[0_0_15px_rgba(245,158,11,0.4)] hover:shadow-[0_0_25px_rgba(245,158,11,0.6)] flex items-center gap-2"
            >
              Send <Send size={16} />
            </button>
