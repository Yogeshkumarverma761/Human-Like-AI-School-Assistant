...
import { Send, Mic, MicOff, Settings, Paperclip, GraduationCap, Users, Shield, BookOpen } from 'lucide-react';
...
const ChatInterface = () => {
  ...
  const sendMessage = async (text = input) => {
    ...
    try {
      const contextPrefix = `[System: The current authenticated user is ${user.name}, a ${user.role}.]\n`;
  ...
  return (
    <div className="flex w-full h-screen p-4 md:p-8 max-w-7xl mx-auto gap-6 text-slate-200">
      
      {/* Sidebar: Avatar & Settings */}
      <motion.div 
        initial={{ opacity: 0, x: -50 }}
        animate={{ opacity: 1, x: 0 }}
        className="hidden md:flex flex-col w-1/3 p-6 glass-panel rounded-3xl"
      >
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <div className="mt-4 px-4 py-2 border rounded-full text-xs font-semibold tracking-wider flex items-center gap-2"
               style={{ backgroundColor: 'var(--theme-color-1)', borderColor: 'var(--theme-color-2)' }}>
            <Icon size={14} />
            {user.name} ({user.role})
          </div>
        </div>
        ...
        <div className="mt-auto space-y-4 pt-6 border-t border-white/10">
          ...
          <div>
            ...
            <select 
              ...
              className="w-full bg-black/30 border border-white/10 rounded-xl p-3 outline-none transition-colors focus:border-white/50"
            >
      ...
      <motion.div 
        ...
        className="flex-1 flex flex-col overflow-hidden glass-panel rounded-3xl"
      >
        {/* Header (Mobile only) */}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 opacity-50 text-center">
              ...
              <p className="text-sm mt-2">I am ready to assist you as a {user.role}.</p>
            </div>
          )}
          <AnimatePresence>
            {messages.map((msg, index) => (
              ...
              >
                {msg.sender === 'ai' && (
                  <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-white font-bold text-xs shadow-lg"
                  ...
                  <div 
                    className={`p-4 ${
                      msg.sender === 'user' 
                        ...
                        : 'glass-card rounded-2xl rounded-tl-sm border-l-2 shadow-lg'
                    }`}
                    style={msg.sender === 'ai' ? { borderLeftColor: 'var(--theme-glow)' } : {}}
                  >
        ...
        {/* Input Box */}
        <div className="p-6">
          <div className="relative flex items-center bg-black/40 border border-white/10 rounded-2xl p-2 shadow-lg backdrop-blur-md focus-within:border-white/30 transition-colors">
            ...
            <button 
              onClick={() => sendMessage()}
              className="px-6 py-3 text-white rounded-xl font-medium transition shadow-lg flex items-center gap-2 hover:opacity-90"
